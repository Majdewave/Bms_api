    ...
    public async Task<IActionResult> GetByClient(Guid clientId)
    {
        var consents = await _context.ClientConsents
            ...
            .Select(c => new ConsentResponse
            {
                ...
                DoctorSignatureUrl = c.Appointment.Staff.User.StampUrl,
                SignedAt = c.SignedAt,
    ...
    public async Task<IActionResult> GetById(Guid id)
    {
        var consent = await _context.ClientConsents
            ...
            .Select(c => new ConsentResponse
            {
                ...
                ConsentContent = c.ConsentContent,
                ClientSignatureUrl = c.ClientSignatureUrl,
                DoctorSignatureUrl = _context.BusinessUsers
                 .Select(bu => new { bu.Id, bu.UserId, bu.User.StampUrl })
                 .FirstOrDefault().StampUrl,
                SignedAt = c.SignedAt,
                CreatedAt = c.CreatedAt,
                ServiceName = c.Appointment != null && c.Appointment.Service != null
                    ? c.Appointment.Service.Name
    ...
    public async Task<IActionResult> GetSignedConsentPdf(Guid id)
    {
        ...
        if (appointment.StaffId != null)
        {
            var bu = await _context.BusinessUsers
                .IgnoreQueryFilters().Include(b => b.User)
                .FirstOrDefaultAsync(b =>
                    b.Id == appointment.StaffId &&
        ...
        var pdf = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().Column(col =>
                {
                    ...
                    col.Item().AlignCenter().Text(tenant?.Name ?? string.Empty)
                        .Style(HebrewStyle()).FontSize(16).Bold();

                    col.Item().AlignCenter().Text("טופס הסכמה חתום")
                        .Style(HebrewStyle()).FontSize(24).Bold();
                    ...
                    col.Item().PaddingTop(15).Border(1).Padding(12).Column(section =>
                    {
                        ...
                        section.Item().PaddingTop(10).AlignRight().Column(col =>
                        {
                            ...
                            var paragraphs = ExtractAll(content, "<div>", "</div>");
                    ...
                    col.Item().PaddingTop(20).Row(row =>
                    {
                        ...
                        {
                            sig.Spacing(4);
                            sig.Item().AlignCenter().Text("חתימת מטופל").Style(HebrewStyle()).Bold();
                            ...
                            sig.Item().PaddingTop(6).AlignCenter().Text(client.FullName)
                                .FontFamily("Noto Sans Hebrew")
                                .DirectionFromRightToLeft()
                                .FontSize(10);
                        });
                        ...
                        {
                            sig.Spacing(6);
                            sig.Item().AlignCenter().Text("חותמת רופא").Style(HebrewStyle()).Bold();
                            ...
                            sig.Item().PaddingTop(6).AlignCenter().Text(doctor?.FullName ?? string.Empty)
                                .FontFamily("Noto Sans Hebrew")
                                .DirectionFromRightToLeft()
                                .FontSize(11);
                        });
